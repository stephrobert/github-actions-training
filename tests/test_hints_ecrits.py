...
def test_quatre_indices_de_cout_croissant(rel: str) -> None:
    """Quatre paliers, du plus vague au plus explicite.

    Quatre et non trois, contrairement aux catalogues Terraform et Linux : le
    CLAUDE.md de ce dépôt le fixe, et le quatrième palier a ici un emploi
    précis, nommer l'outil qui montre la faute (`zizmor`, `actionlint`) quand
    tout semble juste sans l'être.
    """
    couts = [i["cost"] for i in indices_en_clair(rel)]
    assert couts == COUTS_ATTENDUS, (
        f"{rel} : les coûts des indices valent {couts}, attendu "
        f"{COUTS_ATTENDUS}. Quatre paliers du plus vague au plus explicite."
    )
